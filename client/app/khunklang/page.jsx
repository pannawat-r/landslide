...
export default function Angkhang() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://127.0.0.1:5000/api/khunklang')
    ...
    return (
        <>
            <Header />
            <p className='text-center text-2xl py-5'>ขุนกลาง (ตำบลบ้านหลวง อำเภอจอมทอง จังหวัดเชียงใหม่)</p>

            <div className="container mx-auto">
                {/* Map */}
                <div className="relative z-0">
                    <div className="w-full h-[50rem]">
                        <Map position={[18.540612605384062, 98.52361195667108]} zoom={14}></Map>
                    </div>
                </div>
